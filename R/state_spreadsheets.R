create_state_spreadsheet <- function(data, filename) {
  ...
  source <- paste(
    ...
    fmt_txt(
      "Economic Policy Institute Minimum Wage Simulation Model; see Technical Methodology by Cooper, Mokhiber, and Zipperer (2019)."
    )
  )
  ...
  us_notes <- paste(
    ...
    fmt_txt(
      "Values reflect the population estimated to be affected by the proposed change in the federal minimum wage. Wage changes resulting from scheduled state and local minimum wage laws are accounted for by EPI's Minimum Wage Simulation Model. Totals may not sum due to rounding. Shares calculated from unrounded values. Directly affected workers will see their wages rise as the new minimum wage rate will exceed their current hourly pay. Indirectly affected workers have a wage rate just above the new minimum wage (between the new minimum wage and 115% of the new minimum). They will receive a raise as employer pay scales are adjusted upward to reflect the new minimum wage. Values marked * cannot be displayed because of sample size restrictions."
    )
  )
...
add_state_worksheet <- function(workbook, state, data, notes, source) {
  ...
  if (state == "United States") worksheet_name = "U.S. Total" else
...
add_state_intro_worksheet <- function(workbook) {
  workbook %>%
    ...
    wb_add_data(
      x = "Estimated effects of the Raise the Wage Act of 2025",
      start_row = 1
    ) %>%
    wb_add_data(x = "Economic Policy Institute, March 2025", start_row = 2) %>%
    wb_add_data(
      x = "This spreadsheet contains national and state-specific estimates of the effects of the Raise the Wage Act of 2025, as estimated by the Economic Policy Institute Minimum Wage Simulation Model.",
      start_row = 4
    ) %>%
    wb_add_data(
      x = "Detailed estimates for California, District of Columbia, Hawaii, and Washington are unavailable because of the small number of workers affected by the policy in these states.",
      start_row = 6
    ) %>%
    wb_add_data(x = "CITATIONS", start_row = 8) %>%
    wb_add_data(
      x = "Please cite the estimates in this spreadsheet as \"Estimated effects of Raise the Wage Act of 2025,\" Economic Policy Institute Minimum Wage Simulation Model, March 2025.",
      start_row = 9
    ) %>%
    wb_add_data(x = "ASSUMPTIONS", start_row = 11) %>%
    wb_add_data(
      x = "The estimates are for the year 2030, when the policy's regular minimum wage is $17 and the tipped minimum wage is $15.",
      start_row = 12
    ) %>%
    wb_add_data(
      x = "The underlying wage distribution is based on the 2024 Current Population Survey.",
      start_row = 13
    ) %>%
    wb_add_data(
      x = "The simulation assumes nominal wage growth will be at a 3.5% annual rate between 2024 and 2025, and at a annual rate of 0.8% plus projected CPI growth in subsequent years.",
      start_row = 14
    ) %>%
    wb_add_data(
      x = "The simulation accounts for estimated effects of projected state minimum wages between 2025 and 2030.",
      start_row = 15
    ) %>%
    wb_add_data(x = "DOCUMENTATION", start_row = 17) %>%
    wb_add_data(
      x = "To read more about the EPI Minimum Wage Simulation Model, see",
      start_row = 18
    ) %>%
    wb_add_data(
      x = "* the description in Cooper, Mokhiber, Zipperer (2019): https://www.epi.org/publication/minimum-wage-simulation-model-technical-methodology/",
      start_row = 19
    ) %>%
    wb_add_data(
      x = "* a Stata implementation of the simulation model: https://github.com/Economic/min_wage_sim",
      start_row = 20
    ) %>%
    wb_add_data(
      x = "* the code used to produce these estimates: https://github.com/Economic/rtwa_2025_17",
      start_row = 21
    ) %>%
